...
input_required_details() {
    ...
    read -p "Enter the token name (e.g., ToanBm Token): " TOKEN_NAME
    read -p "Enter the token symbol (e.g., TBM): " TOKEN_SYMBOL
...
EOL
    ...
    DEPLOY_OUTPUT=$(forge create "$SCRIPT_DIR/src/ToanBM.sol:ToanBM" \
        --rpc-url rpc_url \
